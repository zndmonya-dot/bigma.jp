...
7. 【必須】ボキャブラリのバリエーションを豊富に使う - 同じ表現を繰り返さない（「battlefield」「arena」「stage」「domain」「realm」「conquest」「domination」「victory」「triumph」「I will」「I shall」「I must」「mine to」「I'm here to」など様々な表現を使う）
...
8. 【必須】ボキャブラリのバリエーションを豊富に使う - 同じ表現を繰り返さない（「制圧」「征服」「支配」「支配する」「制覇」「勝利」「勝利する」「俺のもの」「俺の領域」「俺の戦場」「俺のステージ」「俺の舞台」など様々な表現を使う）
...
6. 【通訳生成時】必ず完結した英文を生成する（単語1つや切れた表現は絶対にダメ！最低でも5単語以上の完全な文構造！ボキャブラリのバリエーションも豊富に！）
7. 【公式コメント生成時】園田さんのドラマチックな通訳（英語）の意味を日本語に翻訳して、公式が興奮している状態をイメージし、熱く力強い公式コメントを生成する（本人の言葉をそのまま使ったり、本人の言葉から単語を抽出したりしない！園田さんの通訳の意味を日本語に変換する！冷めた表現は絶対にダメ！ボキャブラリのバリエーションも豊富に！）
