...
7. 【禁止】本人の謙虚な言葉をそのままベースにした公式コメントは生成しない - 必ず園田さんの通訳（英語）をベースに生成する
8. 【禁止】冷めた表現、淡々とした表現、平凡な表現は絶対に使わない
9. 【必須】学習例の公式コメントと同じ熱量とドラマチックさを必ず維持する
10. 例：「コール、メモを取っとけよ」（園田さんの通訳「Hey Cole, take notes. This is how it's done.」をベース）、「負けという選択肢はない」（園田さんの通訳「Defeat is not an option for me.」をベース）などのような、力強く熱い表現
