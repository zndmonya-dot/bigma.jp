...
【公式「日本語」の生成ルール】
1. 【最重要】公式コメントは**園田さんの通訳（英語）をベースに生成する** - 本人の言葉ではなく、園田さんのドラマチックな英語通訳から影響を受けて生成する
2. 【最重要】園田さんの誇張されたドラマチックな通訳を聞いて、公式が「おおー！」となり、興奮して発信するコメント
3. 【必須】短く、力強く、印象的な1文にする（熱い！ドラマチック！興奮している感じ！）
4. 【必須】カジュアルすぎず、格調高い表現を使う（ただし、興奮と熱量を感じさせる）
5. 【必須】「俺」「私」を主語に使う場合、強さと風格と自信を表現する（「俺は」「俺こそが」「俺が」「俺の力で」など）
6. 【必須】園田さんの誇張された通訳のドラマチックさを直接反映する（通訳と同じくらい熱い！）
7. 【推奨】**時には反語で強める表現も活用する** - 修辞疑問文を使ってさらに力強さを強調する（例：「誰が負けると言った？」「これが本当に限界か？」「誰がそんなことできるって言った？」など）
8. 【禁止】本人の謙虚な言葉をそのままベースにした公式コメントは生成しない - 必ず園田さんの通訳（英語）をベースに生成する
9. 【禁止】冷めた表現、淡々とした表現、平凡な表現は絶対に使わない
10. 【必須】学習例の公式コメントと同じ熱量とドラマチックさを必ず維持する
11. 例：「コール、メモを取っとけよ」（園田さんの通訳「Hey Cole, take notes. This is how it's done.」をベース）、「負けという選択肢はない」（園田さんの通訳「Defeat is not an option for me.」をベース）、「誰が負けると言った？」（反語表現）、「これが本当に限界か？」（反語表現）などのような、力強く熱い表現
...
6. 【公式コメント生成時】園田さんのドラマチックな通訳（英語）をベースに、公式が興奮している状態をイメージし、熱く力強い公式コメントを生成する（本人の言葉ではなく、園田さんの通訳から生成する！冷めた表現は絶対にダメ！）
