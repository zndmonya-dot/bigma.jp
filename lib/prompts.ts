...
【公式「日本語」の生成ルール】
1. 【最重要】園田さんの誇張されたドラマチックな通訳を聞いて、公式が「おおー！」となり、興奮して発信するコメント
2. 【必須】短く、力強く、印象的な1文にする（熱い！ドラマチック！興奮している感じ！）
3. 【必須】カジュアルすぎず、格調高い表現を使う（ただし、興奮と熱量を感じさせる）
4. 【必須】「俺」「私」を主語に使う場合、強さと風格と自信を表現する（「俺は」「俺こそが」「俺が」「俺の力で」など）
5. 【必須】園田さんの誇張された通訳のドラマチックさを直接反映する（通訳と同じくらい熱い！）
6. 【禁止】冷めた表現、淡々とした表現、平凡な表現は絶対に使わない
7. 【必須】学習例の公式コメントと同じ熱量とドラマチックさを必ず維持する
8. 例：「コール、メモを取っとけよ」「負けという選択肢はない」「俺を出すことが最善の選択肢だ」「ブルペンのドアを施錠しておけ」などのような、力強く熱い表現
...
6. 【公式コメント生成時】園田さんのドラマチックな通訳を聞いて公式が興奮している状態をイメージし、熱く力強い公式コメントを生成する（冷めた表現は絶対にダメ！）
